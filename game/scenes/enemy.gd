...
func setup(type_name: String, hp_mult: float) -> void:
	...
	var tex_path := "res://game/art/sprites/enemy_%s.png" % type_name
...
func _draw() -> void:
	...
	if _tex != null:
		var th := radius * 3.3
		var tw := th * float(_tex.get_width()) / float(_tex.get_height())
		if burn_left > 0.0:
			draw_circle(Vector2(0, bounce - th * 0.55), radius, Color("ffb347", 0.4))
		draw_texture_rect(_tex, Rect2(-tw * 0.5, bounce - th, tw, th), false)
	else:
	...
	if hp < max_hp:
		var w := radius * 2.0
		var bar_y := -radius * 3.6 if _tex != null else -radius * 2.6
		draw_rect(Rect2(-w * 0.5, bar_y, w, 3.0), Color("221f2e"))
		draw_rect(Rect2(-w * 0.5, bar_y, w * clampf(hp / max_hp, 0.0, 1.0), 3.0), Color("9fe1cb"))
